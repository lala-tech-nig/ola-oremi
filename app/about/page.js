...
import { ShieldCheck, Flag, Wrench, Clock, Download, ChevronRight, Mail, Phone, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900">
      <header className="px-8 md:px-20 py-12 max-w-7xl mx-auto">
        ...
        <p className="text-gray-500 max-w-2xl leading-relaxed">Building the Future of Nigeria with integrity, innovation, and engineering precision.</p>
      </header>

      <section className="px-8 md:px-20 py-12 max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <div className="relative rounded-3xl overflow-hidden shadow-2xl">
          <img src="https://images.unsplash.com/photo-1541888946425-d81bb19480c5?auto=format&fit=crop&q=80" alt="Engineers on site" className="w-full h-[500px] object-cover" />
        ...
        <div>
          ...
          <div className="space-y-6 text-sm leading-relaxed text-gray-600">
            <p><span className="font-bold text-blue-700">OLA-OREMI NIGERIA LIMITED</span> is a premier indigenous construction firm dedicated to transforming the Nigerian landscape. We combine deep local expertise with global technology standards to deliver world-class infrastructure projects.</p>
            <p>With a focus on civil engineering, piling, and property development, we have established a reputation for reliability. Our approach is simple: safety first, quality always, and timely delivery on every project.</p>
            <p>Registered under <span className="font-bold">RC NO: 8032136</span>, we are fully compliant with all regulatory standards and pride ourselves on being a 100% Nigerian-owned entity contributing to national development.</p>
          </div>
          <div className="flex gap-4 mt-10">
            <button className="bg-[#1E56C8] text-white px-8 py-3 rounded-md text-sm font-bold flex items-center gap-2 hover:bg-blue-700 transition">View Our History <ChevronRight size={16} /></button>
            <button className="border border-gray-200 text-gray-700 px-8 py-3 rounded-md text-sm font-bold flex items-center gap-2 hover:bg-gray-50 transition">Download Profile <Download size={16} /></button>
          </div>
        </div>
      </section>
      ...
      <section className="px-8 md:px-20 py-24">
        <div className="max-w-5xl mx-auto bg-white rounded-[40px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.08)] p-12 md:p-20 flex flex-col md:flex-row items-center gap-16 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <blockquote className="text-2xl md:text-3xl font-medium text-[#0A1D37] leading-relaxed mb-8 italic">"Our foundation is built on integrity and engineering precision. We don't just construct buildings; we build trust that stands the test of time."</blockquote>
      ...
      <section className="mx-8 md:mx-20 mb-20">
        <div className="max-w-7xl mx-auto bg-[#1E56C8] rounded-3xl p-12 md:p-16 flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
          ...
          <button className="relative z-10 bg-white text-[#1E56C8] px-10 py-4 rounded-xl font-bold flex items-center gap-3 hover:bg-blue-50 transition">Get a Quote <ArrowRight size={20} /></button>
        </div>
      </section>
    </div>
  );
...
function GlanceCard({ icon, label, value }) {
  return (
    <div className="bg-white p-8 rounded-2xl border border-gray-100 hover:shadow-xl transition-all duration-300">
      <div className="bg-blue-50 w-10 h-10 flex items-center justify-center rounded-lg mb-6">{React.cloneElement(icon, { size: 18 })}</div>
